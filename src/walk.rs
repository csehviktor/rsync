...
pub fn collect_files(root: &Path) -> RsyncResult<Vec<String>> {
...
fn visit(root: &Path, dir: &Path, files: &mut Vec<String>) -> RsyncResult<()> {
    for entry in std::fs::read_dir(dir).unwrap() {
        ...
        if file_name.into_encoded_bytes().first() == Some(&b'.') {
            continue;
        }
...
mod tests {
    ...
    fn walk() {
        ...
        let keys = collect_files(&root).unwrap();
